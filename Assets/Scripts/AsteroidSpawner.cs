...
public class AsteroidSpawner : MonoBehaviour {
    ...
    void Update()
    {
        AsteroidsCount = PlayerShip.GetComponent<PlayerController>().enemyCount;

        if (killedCount == 2)
        {
            PlayerShip.GetComponent<ScenarioController>().PrintMessage("Nice! Now we've got a... Ah, shit!, there's more incoming!");
        }

        /*else
        {
            PlayerShip.GetComponent<PlayerController>().printMessage = false;
        }*/
        ...
        if (PlayerShip.GetComponent<PlayerController>().enemies.Count == 0 && killedCount < 3) {
            killedCount++;
            PlayerShip.GetComponent<PlayerController>().enemyCount += 10;
            IsSpawningFinished = false;
        }

        if (PlayerShip != null && !IsSpawningFinished && killedCount < 3)
        {
            CreatePlayableGrid();
            MarkPlayerSafeArea();
            SpawnNewAsteroids();
    ...
    private Vector3 GetPlayingFieldExtents()
    {
        ...
        Vector3 maxPointVS = new Vector3(1, 4.5f, cameraDistance);
    ...
    private void CreatePlayableGrid()
    {
        ...
        for (int i = 0; i < xSize; ++i)
        {
            ...
            {
                Vector3 cellOffset = new Vector3(i, 2f, j) * PlayableGridCellSize + new Vector3(halfCellSize, playingFieldExtents.y, halfCellSize);
    ...
    private void SpawnNewAsteroids()
    {
        ...
        {
            var asteroid = Instantiate(AsteroidPrefab, asteroidPositions[i], Random.rotation);
            asteroid.GetComponent<AsteroidMovementController>().RegisterPlayer(PlayerShip);
            PlayerShip.GetComponent<PlayerController>().enemies.Add(asteroid);
        }
    }
